...
def processed_data():
    ...
    data['Надпрофессиональный навык'] = data['Надпрофессиональный навык'].apply(eval)
    data["Отрасль деятельности"] = data["Отрасль деятельности"].apply(eval)
    data['Отрасль деятельности'] = data['Отрасль деятельности'].apply(lambda x: ';'.join(x))
...
def update_filter_choices():
    ...
    employment_choices = sorted(data["Вид трудоустройства"].dropna().unique().tolist())
    # industry_choices = sorted(data["Отрасль деятельности"].explode().dropna().unique().tolist())
    industry_choices = sorted(data["Отрасль деятельности"].str.split(';').explode().dropna().unique().tolist())
    skills_choices = sorted(data["Ключевые навыки"].str.split(';').explode().dropna().unique().tolist())
...
with ui.nav_panel('О проекте', icon=icon_svg("circle-info")):
    ...
    <div>
       ...
       <ul>
         ...
         <li><strong>Анализировать данные</strong> 📊 с помощью фильтров (дата, заработная плата, регион, опыт работы и др.).</li>
        ...
        with ui.nav_panel("Одномодальный граф схожести"):
            with ui.layout_columns(col_widths=(3, 9)):
                with ui.card(full_screen=False):
                    ...
                    ui.input_selectize(
                        "node_size_scale_sim", "Масштаб размера узла:",
                        choices=["lin", "log", "pow", "sqrt"], width=250
                    )
                    ui.input_slider(
                        "louvain_resolution_sim", "Разрешение Louvain:",
                        min=0, max=2, value=1, step=0.1, width=250
                    )
                with ui.card(full_screen=True):
                    ui.card_header("🔗 Граф")
                    ...
                    def widget_similarity():
                        ...
                        return Sigma(
                            ...
                            node_size_scale=input.node_size_scale_sim() or "lin",
                            node_metrics={"community": {
